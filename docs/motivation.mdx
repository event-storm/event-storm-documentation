...
Why is it like this? Having a centralized data store, has no **relation with the the data kept**, has to deal with it as a single entry. **The data store and data relation is crucial**(to see the alternatives [see]( #whateventstormcansuggest)). 
...
Redux data store is has official support of integration with React UI library. The Redux is forcing to set a **[Contenxt.Provider](https://reactjs.org/docs/context.html#contextprovider)**. From that point the data access, from any UI component will be managed by React not Redux. 
